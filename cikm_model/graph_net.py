...
class SentRGAT(nn.Module):
    ...
    def forward(self, adjacent_matrix, head_flag_bit_matrix, edge_type_matrix, node_embedding):
        n = adjacent_matrix.size(1)

        # tail_embedding_matrix = head_embedding_matrix = self.t1(self.node_embedding(head_nodes))  # B, N, En
        tail_embedding_matrix = head_embedding_matrix = self.t1(node_embedding)  # B, N, En
        ...
        # head_tail_flag : [ [head, tail, edge], [head, tail, edge] ...]
