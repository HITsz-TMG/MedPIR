...
class SessionProcessor:
    def __init__(self, word_vocab: Vocab, inner_vocab: Vocab,
                 pv_r_u_max_len, r_max_len, task=None):
    ...
    def original_process(self, session, supervised=False):
        ...
        return session_segs, [session_name] * len(session_segs)

    def process(self, session, supervised=False, options: dict = None):
        if self.task != "meddg":
            return self.original_process(session, supervised=supervised)

        session_segs = []  # session_segs is used for final training/test data
        history = []  # history is a temporary buffer
        sentence_cache = []
        speaker = []
        session_name = session[0][4]
        # if options.get("task") == "meddg" and options.get("type") == "test":
        #
        #     for i in range(0, len(session) - 1):
        #         # 之前一轮的对话
        #         pv_r_u = [x[0] for x in session[max(0, i - 1): i + 1]]
        #         history += pv_r_u
        #
        #         response = session[i + 1]
        ...
        for i in range(0, len(session) - 1):
            ...
            if len(speaker) == 0:
                sentence_cache.append(session[0][0])
                role = session[0][3]
                speaker.append(role)
            else:
                role = session[i][3]
                if speaker[-1] == role:
                    sentence_cache[-1] = sentence_cache[-1] + session[i][0]
                else:
                    sentence_cache.append(session[i][0])
                speaker.append(role)
            history += sentence_cache[-2:]

            # pv_r_u = [x[0] for x in session[max(0, i - 1):i + 1]]  # r_{t-1}, u_{t}
            # history += pv_r_u

            response_role = session[i + 1][3]
            if response_role == "patient":
                continue
            ...
            intention_str = "[{}]".format(str(response_type).upper())
            intention_idx = intention2str.index(intention_str)
            intention_spa = [0 if intention_idx != i else 1 for i in range(4)]  # one-hot
...
class SessionDataLoader:
    def __init__(self, sessions, batch_size, session_processor: SessionProcessor,
        ...
        self.mode = mode
        self.shuffle = shuffle
        self.history_sessions = [[[], None] for _ in range(self.batch_size)]
        self.sunset = False
        self.device = device
        self.dtypes = dtypes
        self.supervised = supervised
    ...
    def load_processed_session(self):
        ...
        processed_session, session_name = self.session_processor.process(session, supervised=self.supervised)
        self.session_index += 1
        return processed_session, session_name
    ...
    def __next__(self):
        ...
        for i in range(len(self.history_sessions)):
            if len(self.history_sessions[i][0]) == 0:
                if self.sunset:
                    is_efficient.append(0)
                else:
                    while True:
                        processed_session, session_name = self.load_processed_session()
                    ...
                    if (processed_session is not None) and (len(processed_session) > 0):
                        self.history_sessions[i][0] = processed_session
                        self.history_sessions[i][1] = session_name
                        is_efficient.append(1)
                    else:
                        is_efficient.append(0)
        ...
        self.history_sessions = [[session, name] for session, name in self.history_sessions if len(session) > 0]
        if len(self.history_sessions) == 0:
            raise StopIteration

        batch_sessions = [session[0] for session, name in self.history_sessions]
        batch_session_names = [name for session, name in self.history_sessions]
        self.history_sessions = [[session[1:], name[1:]] for session, name in self.history_sessions]
        ...
        return nn_inputs, is_continuous, is_efficient, batch_session_names
